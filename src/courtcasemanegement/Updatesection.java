...
public class Updatesection extends javax.swing.JFrame {
    ...
    String sectionId = jTextField1.getText().trim();
    String sectionName = jTextField3.getText().trim();
    String priority = jTextField2.getText().trim();

    // Validate input fields
    if (sectionId.isEmpty() || sectionName.isEmpty() || priority.isEmpty()) {
    ...
    try {
        ...
        URL url = new URL("http://localhost:3000/v1/update-section?section=" + sectionId); // Adjust URL if needed
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("PUT"); // Use PUT method for updating
        conn.setRequestProperty("Content-Type", "application/json");
        conn.setDoOutput(true);

        // Send the JSON section data
        try (OutputStream os = conn.getOutputStream()) {
            byte[] input = sectionData.getBytes(StandardCharsets.UTF_8);
            os.write(input, 0, input.length);
        }

        // Check the response from the server
        int responseCode = conn.getResponseCode();
        if (responseCode == 200) { // Section updated successfully
            JOptionPane.showMessageDialog(this, "Section updated successfully.");
        } else {
            // Read the error response from the server if update failed
